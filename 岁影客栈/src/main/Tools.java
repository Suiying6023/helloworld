...
public class Tools {
    ...
    public static boolean login() throws Exception {
        ...
        if (!Constants.users.containsKey(username)) {
            System.out.println(
                    String.format("用户%s不存在，请重新登录或注册", username));
            System.out.println();
            return false;
        }
        ...
        System.out.println(String.format(
                "用户%s,登录成功", Constants.currentUser.getUsername()));
        System.out.println();
        if (Constants.currentUser.isAdmin()) {
            // 如果当前用户是管理员，则调用adminMenu方法进入管理员界面
            System.out.println(
                    "****************尊敬的管理员用户，欢迎进入岁影客栈管理系统****************");
            AdminMenu.adminMenu();
        } else {
    ...
    public static void changePassword() throws Exception {
        ...
        Constants.users.put(
                Constants.currentUser.getUsername(), Constants.currentUser);
